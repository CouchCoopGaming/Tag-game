...
This delta: A run into a crouch walk eases the run into the low stride. The stride then holds. A walk into a crouch walk is unchanged. A run into a still crouch is unchanged. The drop time is unchanged.
