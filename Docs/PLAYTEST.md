...
| **Playground** | Four hopscotch courts, one per corner. West run: spawn SW, mushroom step, hopscotch SW, south bar, soft-play (rung to the 2.00 deck), spine, bars or beams, merry (picnic on the south edge), spine, north bar, arch to hopscotch NW, astro. East forts use that same 2.4 m rung opposite a 1.8 m ladder. East run: hopscotch SE, arch, army, bars onto open kickball, swings (fall tiles clear of the north fence), bars, then knight to the west or the arch into hopscotch NE. Crash bowl stays open. Spawn_NW faces southeast; the torso is north of that pad and off the exit. After pull: **CutArenaBootstrap** Rebuild / **PgkLandmarkPlacer -> Place**. |
...
1. **SW exit.** Spawn_SW faces northeast. Mushroom steps at (4.5, 6.94) sit beside that line, between the pad and hopscotch SW. Feet are on the ground. No arch here: the court and the south bar are already about 2.2 m apart.
