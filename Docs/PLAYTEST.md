...
Branch: `cursor/menu-focus-input-leak-497c` (into `cursor/playground-campus-zones-afc4`). Deeper notes: `Docs/MOVEMENT.md` / `Docs/PLAY-SLICE.md`.
...
| **Playground** | Four hopscotch courts, one per corner. West run: spawn SW, mushroom step, hopscotch SW, a low bench in the 2.2 m gap (the gap stays open), south bar, soft-play (rung to the 2.00 deck), spine, bars or beams, merry (picnic on the south edge), spine, north bar, arch to hopscotch NW, astro. East forts use that same 2.4 m rung opposite a 1.8 m ladder. East run: hopscotch SE, arch, army, bars onto open kickball (a low bench west of the bars, not in the field), swings (fall tiles clear of the north fence), bars, then knight to the west or the arch into hopscotch NE. Crash bowl stays open. Spawn_NW faces southeast; the torso is north of that pad and off the exit. After pull: **CutArenaBootstrap** Rebuild / **PgkLandmarkPlacer -> Place**. |
...
1. **SW exit.** Spawn_SW faces northeast. Mushroom steps at (4.5, 6.94) sit beside that line, between the pad and hopscotch SW. Feet are on the ground. No arch here: the court and the south bar are already about 2.2 m apart. A bench at (9.85, 9) sits in that gap, 0.45 m thick, about 0.9 m off the court and off the bar. The court is still the destination.
2. **West loop.** Hopscotch SW -> south bar (x=11) -> soft-play (tubes, slide, 2.4 m rung, net beam, or the tall net) -> cross the south spine on foot -> middle bars or the beam lane -> merry apron into the bars. Picnic table is on the merry's south edge, not on the apron. Cross the north spine -> north bar -> astro.
...
4. **East loop.** Spawn_SE -> hopscotch SE -> arch at (65.90, 10.5) -> army. Army and knight each have the 2.4 m rung on the west shoulder and the 1.8 m ladder on the net side. Bars at x=62.5 into the open west side of kickball -> swings. A bench at (61.30, 21.05) is west of those bars, south of the Loop E stair and north of the south spine. The diamond stays empty. Swing fall tiles should clear the kickball north fence by about 0.16 m. Continue the bars. Knight is west of those bars (same rung and ladder). The arch at (65.65, 41) is east of them, then hopscotch NE. Spawn_NE faces southwest, into the keep. The shield is north of that pad. Merry, hopscotch, and swings were not moved.
5. **Crash.** Cross the bowl east-west. Both lips and the middle should be open lawn.
6. Play path: hopscotch corners, swing fall tiles clear kickball fence, crash bowl open, three Toy_Bridge path arches grounded, army/knight 2.4 m climb rungs to the 2.00 deck. Sideline benches mark kickball open edge and the SW hopscotch-to-bar gap (no fourth arch).
