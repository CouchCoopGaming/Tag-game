...
| **Playground** | West play places link along **BARS W** (x=11); the merry apron runs under that lane at z 22-26 (the z=24 joint stays mulch). Beam lane x=13.5 stops 0.25 m short of the Loop W tower. East bars at x=62.5 sit on the open kickball edge; beam lane x=60.5 starts 0.25 m north of the Loop E tower. Play places have a 2.4 m rung against the deck; bunkers have a 1.8 m ladder on the net side. Slide pits stay three tiles long and gain a side wing (outer side only on forts, so the wing misses the Conn ramp). After pull: **CutArenaBootstrap** Rebuild / **PgkLandmarkPlacer -> Place**. |
...
1. **Bar drops.** West bars stay at x=11. Apron tiles now cover world z 22, 23, 25, and 26 under the middle spans. The z=24 joint is still mulch so the bar feet stay clear. East bars stay at x=62.5 (the whole bar is on the rubber; the pad starts at x=62). You still cross the ski spines on foot.
2. **Run.** Spawn SW -> hopscotch SW -> south bar -> soft-play (tubes, slide, 2.4 m rung, or the tall net) -> cross the south spine -> middle bars or the beam lane -> merry apron -> cross the north spine -> north bar -> astro. East: army crawl or the 1.8 m ladder -> bars into kickball -> swings -> bars -> knight. The spine corridors themselves stay empty.
...
5. Slide pit is still three tiles long (~2.2 m after the chute, ~0.4 m before the spine). Mid and far tiles have a side wing. Forts only get the outer wing; the inner one would sit on the Conn ramp. Say if the runout still feels short.
6. Army crawl is shifted to local (-2, -4.5): about 1 m south of the stair, west edge short of Spawn_SE. Knight's copy should miss Spawn_NE. Bunker ladder is on the net side of the deck (local 1.20, 0.45).
...
8. **Corners.** Helmet stays (2.5, 51.2) scale 0.30 - that is the largest yaw-0 fit west of Spawn_NW. Foxhole is (69.55, 2.55) scale 0.35 yaw 0, east of Spawn_SE. Tron disc moved to (42.1, 11.5) so it is off Conn_Tron. Ninja rail is grounded and at (19.9, 46.6), west of the chase lane and inside the map. Say if the helmet still reads small.
...
- Round over draws a center card (cursor unlocks for clicks). Solo headline is YOU WIN, YOU LOSE, or DRAW. Couch with a split result stays ROUND OVER and names the winners. **R** / Rematch button starts once; Boot does not also rematch. A second StartRound inside 0.05 s is ignored. **Q** or **Esc** returns to Boot (GameFlow when it is loaded, otherwise a direct scene load). F1-F4 or R leave pause and the round-end state so the countdown is not frozen. Resume and quit click on pause; Rematch/Menu click on results. "No winners" no longer plays the win sting. A loss sting does not say "winner", so it does not play the win tone. Esc on the player-count and mode screens steps back to Boot. Keys 1-4 on the player screen match the four rows: you + 1 bot, or 2-4 humans with the bot off. Direct Play (no GameFlow) pauses on Esc; Q loads Boot. Pause zeroes look and punch, clears a buffered jump, and HUD flashes use scaled time so they freeze. The countdown card names the mode and shows 3, 2, 1. Local punch windup flares the elbow beside the head; the 0.12 s windup is unchanged. The punch-buffer cock from the last pass stays. Look sensitivity is a five-step stub (default 1.8, the camera field). Boot and the pause menu open it; Left/Right change it. Playground music stays silent: only the wav meta exists, and PlayMusic returns when the clip is null. Ski entry still uses TagSfx.EnsureSource.
