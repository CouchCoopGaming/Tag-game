...
This delta: A crouch walk into a run eases the low stride into the run. The run then holds. A crouch walk into a walk is unchanged. A still crouch into a run is unchanged. The drop time is unchanged.
