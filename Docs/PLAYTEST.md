# Tag â€” local playtest (Amaterasu)

## Open

1. Unity Hub â†’ open `C:\Users\Zubal\Dev\Tag-game-playtest` (6000.3.x / 6000.0.x).
2. Open scene **Play** (`Assets/Scenes/Play.unity`) â†’ **Play**.
3. Optional first-time art: **Tag â†’ Ensure URP Pipeline**, then **Tag â†’ Setup Hub Visuals**.

Branch: `cursor/playground-campus-zones-afc4` (integration tip; map 68edd8f + merged tag-loop-kite-cam / PR #11). Deeper notes: `Docs/MOVEMENT.md` / `Docs/PLAY-SLICE.md`.

## Stack snapshot

| Piece | What you get |
|-------|----------------|
| **TP cam** | `TpsMoveCamera` orbit/follow (~boom âˆ’5.2), soft collision, FOV by MoveState + slight speed look-ahead (`lookAheadMax` ~0.9, `speedFovBoostMax` ~3Â°). Look-ahead **direction** is smoothed (a brake used to yaw the aim point in one frame). Strafe roll is lighter so the boom does not orbit and snap back. |
| **Hier dummy** | `DummyAvatarBinder` prefers `*_Hier_Hi` FBX â†’ flat HiPoly â†’ Navy Spade primitive (foam + polymer panels + matte joints; palette aligned with `Tools/Tag/build_mannequin_hier.py` â€” Tan runner / Orange It); `DummyLocomotor` swings limbs when bindable UpperArm/UpperLeg hierarchy exists |
...
| **Ski spines** | Thicker mega-park ski spines (~3 m wide / 0.12 m thick) + zone approach ramps (pad â†’ nearest spine); denser PGK connectors (`CutArenaBootstrap.BuildSkiSpines`) |
...
| **Lunge** | It-only MMB: ~16 m/s, ~0.20 s, **CD ~1.0 s**; TP whipâ†’settle via `LungeProgress` |
...
Punch is **not** a contact aura â€” only active punch hits transfer It (`PunchHitbox`).
...
1. **Bar drops.** West bars stay at x=11 (the mast owns anything nearer around z 12â€“18). The merry apron tiles reach world x=11 at z 23 and 25, so the drop is on the apron. The z=24 joint is mulch on purpose â€” a tile there would bury the bar feet. East bars are at x=62.5, just inside the kickball rubber's open west edge, not on the swing bays. You still cross the ski spines on foot between segments.
2. **Run.** Spawn SW â†’ hopscotch SW (east-west) â†’ south bar â†’ soft-play tubes/slide, or the new rung ladder onto the deck â†’ cross the south spine â†’ middle bars or the beam lane (x=13.5) â†’ merry apron â†’ cross the north spine â†’ north bar â†’ astro loft. East mirror: army crawl â†’ bars (x=62.5) â†’ kickball (open west) â†’ swings â†’ bars â†’ knight, with the east beam lane (x=60.5) beside the bars.
...
6. Army crawl is shifted to local (âˆ’2, âˆ’4.5): about 1 m south of the stair, west edge short of Spawn_SE. Knight's copy should miss Spawn_NE.
7. Kickball fence is north, east, and behind the south goal. South fence is local z=âˆ’4.0 (world z=20): about 0.36 m off the south spine and about 0.13 m behind the goal back, not in the mouth. West side stays open.
8. **Spawns and corners.** SW toys sit west of Spawn_SW. NW toys sit south of Spawn_NW (the old cluster crossed the north map edge). Helmet (2.5, 51.2, scale 0.30) and foxhole (69.8, 2, scale 0.26, yaw 90) are the sizes that stay on the map and off the pads â€” say if they read too small. Knight shield is north of Spawn_NE at (66, 52.2).
...
- Trail Tag self-hit still needs both age and distance grace. Dodge i-frames do not ignore trails. It-emphasis on punch is the ItOnly emitter path; All emitters refresh emphasis on the next tick.
...
Wall-run and wall-climb set a latch on exit (timeout, jump-off, or lost contact). The latch clears on the ground or after ~0.15 s with no wall hit, so air accel cannot restart the timer on the same surface. Climb up-speed (`climbSpeed` 6, decay from 0.16 s, slip Ã— 3.5) reverses before `climbMaxHeight`; the old 7.8 / 0.40 curve hit the height cap at ~0.42 s while still going up, and `ClimbHeightUsed` never cleared on landing. Sprint stays 12 m/s, ski max 24 m/s (run was already raised; ski still wins). Jet stays off.
...
- Prefab/mat dirt after Hub visuals / URP regen â€” do not commit unless intentional.
...
- Trail Tag / Hot Potato polish and AI weave/whiff still need a human feel pass.
