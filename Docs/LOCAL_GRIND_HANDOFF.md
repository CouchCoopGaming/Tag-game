...
`PGK_Slide_TubeDeck_2m` is placed once on soft-play and once on astro. Yaw 90, stem 0, pivot local (0, 0, 5.90). Army and knight keep the straight chute. In the FBX the rise is mesh Z (0.03..2.48) and Y is +/-0.57. Unity's placed AABB matches that (spanY ~1.14), so the crown is not on the 2.00 deck. No placer rotation. Needs an FBX re-export with +Y up. Until then the XZ sleeve is still 3.28 m off the tube street, about 7 m off the ground `Toy_Slide`, and 7.2 m off the dome. East was not given a second tube: that same Z rise would run into the spiral. Props are parked. `Mega_SlideTube` and `PGK_Slide_Tube90` stay unspawned and unscaled. Deck pivots in use are 0.40 / 0.80 / 1.60 / 2.00. A `PGK_Deck_2x2` is 0.08 thick above its pivot, so the tallest walk surface is 2.08. The tallest rise from mulch onto a deck pivot is 2.00. No yaw seats Tube90 or Mega on that grid without a float or a bury. No new deck was added.
...
`PGK_Slide_TubeDeck_2m` is that mesh. It is bound once on soft-play and once on astro. Unity spanY is still ~1.14, so do not add a placer rotation and do not add an east copy. Do not place or scale `Mega_SlideTube` or `PGK_Slide_Tube90`. Props are parked.
