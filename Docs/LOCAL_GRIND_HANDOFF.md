...
- **Arches** (`Toy_Bridge`, feet y=0, deck ~1.05, open span under the middle): NW (7.70, 42) between hopscotch NW and the north bar; NE (65.65, 41) between the east bars and hopscotch NE; SE (65.90, 10.5) between the army net and hopscotch SE. SW has no arch (the court and the south bar are ~2.2 m apart; an arch is 3 m, so piers cannot clear by 1.3 m). A bench at (9.85, 9) yaw 90 sits in that gap, about 0.9 m off the court and the bar.
...
Playtest: spawn SW → mushroom step → hopscotch SW → bench in the 2.2 m gap (9.85, 9) → south bar → soft-play → cross the south spine → bars or beams → merry → cross the north spine → north bar → arch (7.70, 42) → hopscotch NW → astro. East: hopscotch SE → arch (65.90, 10.5) → army (rung or 1.8 m ladder) → bars → kickball (sideline bench at 61.30, 21.05, field still open) → swings → bars, then knight to the west or arch (65.65, 41) into hopscotch NE. Crash cross stays open. The torso is north of Spawn_NW, off the faced exit. Hopscotch, swings, merry, and kickball were not moved.
