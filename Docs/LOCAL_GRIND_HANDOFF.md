...
- **Swings** (67, 31.2): south tiles clear the kickball north fence by ~0.16 m. The swing pad stops at z=34.2, short of the north spine. Kickball's west side stays open.
- **Arches** (`Toy_Bridge`, feet y=0, deck ~1.05, open span under the middle): NW (7.70, 42) between hopscotch NW and the north bar; NE (65.65, 41) between the east bars and hopscotch NE; SE (65.90, 10.5) between the army net and hopscotch SE. SW has no arch (the court and the south bar are ~2.2 m apart).
...
Playtest: spawn SW → mushroom step → hopscotch SW → south bar → soft-play → cross the south spine → bars or beams → merry → cross the north spine → north bar → arch (7.70, 42) → hopscotch NW → astro. East: hopscotch SE → arch (65.90, 10.5) → army (rung or 1.8 m ladder) → bars → kickball → swings → bars, then knight to the west or arch (65.65, 41) into hopscotch NE. Crash cross stays open. The torso is north of Spawn_NW, off the faced exit.
